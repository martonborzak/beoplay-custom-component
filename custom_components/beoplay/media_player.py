...
MIN_TIME_BETWEEN_UPDATES = timedelta(seconds=10) # need this to check on the power state, that doesnt come in the stream of notifications

CHECK_TIMEOUT = 10

SUPPORT_BEOPLAY = SUPPORT_PAUSE | SUPPORT_VOLUME_SET | SUPPORT_VOLUME_MUTE | \
...
class BeoPlay(MediaPlayerDevice):
    ...
    async def async_update_status(self):
        ...
        try:
            response = await self._polling_session.request('get', self._speaker._host_notifications)
            ...
            if response.status == 200:
                ...
                _LOGGER.error("Error %s on %s. Trying one more time",
                              response.status, self._speaker._host_notifications)
